...
static int dsview_log(void *data, int level, const char *format, va_list args)
{
    char *message = g_strdup_vprintf(format, args);
    xlog_writer *writer = xlog_create_writer(data, "srd");
    if (writer && message) {
        if (level <= SRD_LOG_ERR) xlog_err(writer, "%s", message);
        else if (level == SRD_LOG_WARN) xlog_warn(writer, "%s", message);
        else xlog_info(writer, "%s", message);
    }
    xlog_free_writer(writer);
    g_free(message);
    return SRD_OK;
}
SRD_API void ds_srd_log_set_context(xlog_context *context)
{
    srd_log_callback_set(dsview_log, context);
}
